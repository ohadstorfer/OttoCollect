...
interface CollectionFilterCatalogProps {
  countryId: string;
...
export const CollectionFilterCatalog: React.FC<CollectionFilterCatalogProps> = memo(({
  countryId,
  onFilterChange,
  currentFilters,
  isLoading = false,
  ...
  useEffect(() => {
    ...
    if (lastCountryId.current === countryId && initialLoadComplete.current) {
      console.log("CollectionFilterCatalog: Already loaded options for this country, skipping");
      return;
    }
    
    const loadFilterOptionsAndPreferences = async () => {
      console.log("CollectionFilterCatalog: Loading filter options for country:", countryId);
      setLoading(true);
  ...
  const handleGroupModeChange = React.useCallback((mode: boolean) => {
    ...
    if (user?.id) {
      ...
      try {
        sessionStorage.setItem(`groupMode-collection-${countryId}`, JSON.stringify(mode));
      } catch (e) {
...
CollectionFilterCatalog.displayName = 'CollectionFilterCatalog';
