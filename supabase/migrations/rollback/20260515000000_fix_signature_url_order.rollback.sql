-- ROLLBACK for 20260515000000_fix_signature_url_order.sql
--
-- Use this script to revert the signature URL order fix and restore the views
-- to the exact state defined in 20260330_add_turk_catalog_label_to_countries.sql.
--
-- This is NOT a normal migration. It is meant to be pasted manually into the
-- Supabase SQL Editor (or piped through psql) only if the fix migration breaks
-- something in production. It is safe to run multiple times.
--
-- After running, validate with:
--   SELECT extended_pick_number, signatures_front, signatures_front_urls
--   FROM enhanced_detailed_banknotes
--   WHERE country = 'Jordan' AND extended_pick_number = '2a';
-- The rolled-back (buggy) state returns urls in PK order (Al-Haimak first),
