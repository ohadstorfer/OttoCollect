-- Add trigger function to award points for blog comments
...
-- Create trigger for blog comment points
DROP TRIGGER IF EXISTS award_points_for_blog_comment_trigger ON blog_comments;
CREATE TRIGGER award_points_for_blog_comment_trigger
  AFTER INSERT ON blog_comments
  FOR EACH ROW
  EXECUTE FUNCTION award_points_for_blog_comment();

-- Update the recalculate function to include blog comment points
...
DECLARE
    ...
    blog_post_points INTEGER;
    blog_comment_points INTEGER;
...
BEGIN
    ...
    LOOP
        ...
        -- 7. Points for blog posts (5 points each)
        SELECT COUNT(*) * 5 INTO blog_post_points
        FROM blog_posts
        WHERE author_id = user_record.id;
        
        total_points := total_points + blog_post_points;
        
        -- 8. Points for blog comments (1 point each)
        SELECT COUNT(*) INTO blog_comment_points
        FROM blog_comments
        WHERE author_id = user_record.id;
        
        total_points := total_points + blog_comment_points;
        
        -- 9. Points for being admin (500 points, one time)
