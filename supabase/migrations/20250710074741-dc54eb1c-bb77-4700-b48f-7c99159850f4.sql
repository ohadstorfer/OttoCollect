...
-- Create a comprehensive function to award badges based on current user achievements
CREATE OR REPLACE FUNCTION award_user_badges_comprehensive()
      ...
      BEGIN
        ...
        IF should_award AND NOT EXISTS (
          ...
          RAISE NOTICE 'Awarded badge % to user % (% >= %)', 
            badge_record.name, user_record.username, current_value, badge_record.threshold_value;
        END IF;
      END;
    END LOOP;
...
SELECT * FROM award_user_badges_comprehensive();
