-- Fix the handle_old_image_cleanup function to properly detect and queue image changes
-- This migration will completely replace the function with improved logic

-- First, drop the old function completely
DROP FUNCTION IF EXISTS public.handle_old_image_cleanup() CASCADE;

-- Now create the fixed version with better change detection
...
BEGIN
  ...
  RAISE NOTICE 'Function handle_old_image_cleanup has been updated with improved change detection';
END $$; 